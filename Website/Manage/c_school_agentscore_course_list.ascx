<%@ Control Language="VB" AutoEventWireup="false" CodeFile="c_school_agentscore_course_list.ascx.vb" Inherits="c_school_agentscore_course_list" %>
<br />
<div dir="rtl" style="margin-left: 10px; width: 742px; margin-right: 10px; text-align: justify;">
    ...
    <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AllowSorting="True"
        ...
        <Columns>
            ...
            <asp:BoundField DataField="realname" HeaderText="معلم" SortExpression="realname" />
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:Khatam_Site %>"
        ProviderName="<%$ ConnectionStrings:Khatam_Site.ProviderName %>" SelectCommand="SELECT school_Class.title AS class_title, school_Lesson.title AS Lesson_title, school_Lesson.unit, school_Classroom_select.id, school_Class.id_school_year, school_Teacher.id AS school_Teacher_id, school_Teacher.Fname + ' ' + school_Teacher.Lname AS realname FROM school_Class INNER JOIN school_Classroom_select ON school_Class.id = school_Classroom_select.school_class_id INNER JOIN school_Lesson ON school_Classroom_select.school_lesson_id = school_Lesson.id INNER JOIN school_Teacher ON school_Classroom_select.school_teacher_id = school_Teacher.id WHERE (school_Class.id_school_year = @id_school_year)">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:Khatam_Site %>"
        ProviderName="<%$ ConnectionStrings:Khatam_Site.ProviderName %>" SelectCommand="SELECT school_Lesson.title, school_course_personal.id AS id_school_course_personal, school_Lesson.id, school_course_personal.school_teacher_id, school_course_personal.school_year_id, school_Lesson.unit FROM school_course_personal INNER JOIN school_Lesson ON school_course_personal.school_lesson_id = school_Lesson.id WHERE (school_course_personal.school_year_id = @school_year_id)">
        <SelectParameters>
