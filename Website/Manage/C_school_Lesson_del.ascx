<%@ Control Language="VB" AutoEventWireup="false" CodeFile="C_school_Lesson_del.ascx.vb" Inherits="C_school_Lesson_del" %>
<div id="MSG1" runat="server" dir="rtl" style="border-right: red 2px solid; border-top: red 2px solid;
    ...
    <div style="padding-right: 5px; margin-top: 5px; float: right; width: 493px; color: red;
        ...
        توجه!<br />
        <span style="font-size: 9pt">امکان حذف درس مورد نظر به جهت ارائه&nbsp; وجود ندارد،
            لطفا ابتدا با ویرایش این درس را از برنامه کلاسی و ارائه واحد افرادی حذف نمایید.</span></div>
    <br />
</div>
<div id="MSG3" runat="server" dir="rtl" style="border-right: red 2px solid; border-top: red 2px solid;
    ...
    <div style="padding-right: 5px; margin-top: 5px; float: right; width: 493px; color: red;
        padding-top: 5px">
        اخطار!<br />
        <span style="font-size: 9pt"> آیا برای
            حذف درس مطمئن هستید؟<br />
            <br />
...
<div dir="rtl" style="margin-left: 10px; width: 742px; margin-right: 10px" id="Div_Lessons" runat="server">
    <asp:GridView ID="GridView1" runat="server" AllowPaging="True" AllowSorting="True"
        ...
        DataSourceID="SqlDataSource1" PageSize="50" Width="742px">
        <Columns>
            <asp:BoundField DataField="school_Classroom_select_id" HeaderText="کد ارائه" InsertVisible="False"
                SortExpression="school_Classroom_select_id" />
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        SelectCommand="SELECT school_branch.title AS school_branch_title, school_Category.title AS school_Category_title, school_base_cat.title AS school_base_cat_title, school_Class.title AS school_Class_title, school_Class.id, school_Lesson.id AS school_lesson_id, school_Lesson.title AS school_lesson_title, school_Classroom_select.id AS school_Classroom_select_id FROM school_Lesson INNER JOIN school_Classroom_select ON school_Lesson.id = school_Classroom_select.school_lesson_id INNER JOIN school_Class INNER JOIN school_Base ON school_Class.id_school_base = school_Base.id INNER JOIN school_base_cat ON school_Base.year_number = school_base_cat.id INNER JOIN school_Category ON school_Base.id_school_category = school_Category.id INNER JOIN school_branch ON school_Category.id_school_branch = school_branch.id ON school_Classroom_select.school_class_id = school_Class.id WHERE (school_Lesson.id = @school_lesson_id)">
        <SelectParameters>
            <asp:QueryStringParameter Name="school_lesson_id" QueryStringField="id" />
        </SelectParameters>
    </asp:SqlDataSource>
    <br />
    <br />
    <asp:GridView ID="GridView2" runat="server" AllowPaging="True" AllowSorting="True"
        AutoGenerateColumns="False" BorderColor="#BDC6E0" BorderStyle="Solid" BorderWidth="1px"
        DataSourceID="SqlDataSource2" PageSize="50" Width="742px">
        <Columns>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" 
        SelectCommand="SELECT school_course_personal.school_lesson_id AS Expr1, school_Lesson.id AS school_lesson_id, school_Lesson.title, school_course_personal.id FROM school_Lesson INNER JOIN school_course_personal ON school_Lesson.id = school_course_personal.school_lesson_id WHERE (school_course_personal.school_lesson_id = @school_lesson_id)">
        <SelectParameters>
            <asp:QueryStringParameter Name="school_lesson_id" QueryStringField="id" />
        </SelectParameters>
    </asp:SqlDataSource>
