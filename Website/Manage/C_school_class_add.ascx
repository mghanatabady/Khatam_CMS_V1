<%@ Control Language="VB" AutoEventWireup="false" CodeFile="C_school_class_add.ascx.vb" Inherits="C_school_class_add" %>
<br />
        ...
        <div style="margin-left: 9px; width: 724px; margin-right: 9px; background-repeat: repeat-x">
            <div style="margin-top: 5px; float: left; width: 395px; margin-right: 5px; text-align: right">
                <div class="from_div_title">
                    <asp:DropDownList ID="DropDownList1" runat="server" CssClass="from_txt" DataSourceID="SqlDataSource1" DataTextField="title" DataValueField="id" Width="165px">
                    </asp:DropDownList><asp:SqlDataSource ID="SqlDataSource1" runat="server" 
                        SelectCommand="SELECT school_base_cat.title + '  ' + school_branch.title + '  ' + school_Category.title AS title, school_base.id FROM school_branch INNER JOIN school_Category ON school_branch.id = school_Category.id_school_branch INNER JOIN school_base ON school_Category.id = school_base.id_school_Category INNER JOIN school_base_cat ON school_base.year_number = school_base_cat.id ORDER BY school_base.year_number, title, school_Category.title">
                    </asp:SqlDataSource>
            ...
            <div style="margin-top: 5px; width: 320px">
                <div dir="rtl" class="from_div_title">
                    <span class="from_div_title">پایه</span></div>
                <div class="from_div_title">
