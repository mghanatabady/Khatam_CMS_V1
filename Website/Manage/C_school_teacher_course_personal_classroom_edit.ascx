<%@ Control Language="VB" AutoEventWireup="false" CodeFile="C_school_teacher_course_personal_classroom_edit.ascx.vb" Inherits="manage_C_school_teacher_course_personal_classroom_edit" %>
        ...
        <pdc:PersianDateValidator ID="PersianDateValidator1" runat="server" 
            ...
            <div style="width: 720px; text-align: justify;" dir="rtl">
                ...
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:portal1ConnectionString2 %>" 
                    ...
                    SelectCommand="SELECT school_course_personal_student.school_course_personal_id, users.fname, users.lname, users.id, users.lname + ' , ' + users.fname AS realname FROM school_course_personal_student INNER JOIN users ON school_course_personal_student.school_student_id = users.id WHERE (school_course_personal_student.school_course_personal_id = @school_course_personal_id)">
                    <SelectParameters>
                        <asp:QueryStringParameter Name="school_course_personal_id" 
                            QueryStringField="id" />
                    </SelectParameters>
                </asp:SqlDataSource>
                <br />
                ...
                <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:portal1ConnectionString2 %>" 
                    
                    SelectCommand="SELECT school_Score.id, school_Score.value, school_Score.score_cat_id, school_Score.score_type_type, school_Score.classroom_select, school_Score.student_id, school_Score.school_course_personal FROM school_Score INNER JOIN school_course_personal ON school_Score.school_course_personal = school_course_personal.id WHERE (school_course_personal.id = @school_classroom_select_id)">
                    <SelectParameters>
                        <asp:QueryStringParameter Name="school_classroom_select_id" 
                            QueryStringField="id" />
                    </SelectParameters>
                </asp:SqlDataSource>
            </div>
